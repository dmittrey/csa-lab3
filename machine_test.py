...
class ControlUnitTests(unittest.TestCase):
    ...
    def test_DoSaveContext_SaveAllRegisters(self):
        ...
        registers = control_unit.save_context()

        self.assertEqual(control_unit.in_interrupt_context, True)
    ...
    def test_DoRestoreContext_RestoreAllRegisters(self):
        control_unit = ControlUnit()
        control_unit.in_interrupt_context = True
        registers = {
            'PCWrite': 1,
            'IRWrite': 1,
            'ALUSrcA': 1
        ...
        control_unit.restore_context(registers)

        self.assertEqual(control_unit.in_interrupt_context, False)
...
class InstructionPerformTests(unittest.TestCase):
    def test_ADDIFirstTick(self):
        ...
        data_path.Memory.memory[0] = 5488
        data_path.Register_File.inner_registers[2] = 5

        control_unit.attach_wires(data_path.control_wires)
        control_unit.change_state(data_path, tick_valves)

        self.assertEqual(data_path.ALU.get_register('srcA'), 5)
        self.assertEqual(data_path.ALU.get_register('srcB'), 5)
        self.assertEqual(data_path.ALU.get_register('Result'), 10)
    ...
    def test_ADDISecondTick(self):
        ...
        self.assertEqual(data_path.Register_File.get_register('WD'), 10)
        self.assertEqual(data_path.Register_File.get_register('A3'), 7)
        self.assertEqual(data_path.ALU.get_register('srcA'), 0)
        self.assertEqual(data_path.ALU.get_register('srcB'), 1)
        self.assertEqual(data_path.ALU.get_register('Result'), 1)
    ...
    def test_ADDIThirdTick(self):
        ...
        self.assertEqual(data_path.PC.state, 2)
